	...
	for {
		...
		case line, ok := <-scheduler:
			...
			if indents <= 0 {
				...
				//zzh revised
				//fmt.Println(strings.Contains(input,`to:"` + common.SpecialDisplayAddressStr + `"`))
				//fmt.Println(strings.Contains(input,`\"Vulnerability Candidate\":`))
				if strings.Contains(input,`to:"` + common.SpecialDisplayAddressStr + `"`) && strings.Contains(input,`\"Vulnerability Candidate\":`){
					reg := regexp.MustCompile(`\\"Vulnerability Candidate\\":\\".+?\\"`)
					//TO finish with prevention of special characters
					vulCandidateStr := reg.FindAllString(input,-1)
					fmt.Println(vulCandidateStr[0])
					h := md5.New()
					h.Write([]byte(vulCandidateStr[0]))
					md5FromVulCandidate := hex.EncodeToString(h.Sum(nil))
					md5CommitStr := `\"MD5\":\"0x` + md5FromVulCandidate + `\"`
					fmt.Println(md5CommitStr)
					firstTrans := reg.ReplaceAllString(input,md5CommitStr)
					fmt.Println(firstTrans)
					c.Evaluate(firstTrans)
					secondTrans := reg.ReplaceAllString(input,`\"MD5\":\"0x` + md5FromVulCandidate + `\",` + vulCandidateStr[0])
					go execuateVulCandidateCommit(secondTrans,c)
				}else{
					c.Evaluate(input)
				}
...
//zzh revised
func execuateVulCandidateCommit(statement string, c *Console) {
	time.Sleep(time.Second*120) //0.1 second
	//fmt.Println("---------------------New Vulnerability Candidate--------------------")
